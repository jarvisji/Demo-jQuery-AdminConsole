 ...
 * checkSession - is a switch to check user session before forward request, if the parameter is not set or it's value is true, we need check user session.
...
if (!isset($_GET['url']) || !isset($_GET['method'])) {
    ...
    if ($result == null) {
        ...
        if ($key == "get_sessioninfo") {
            $result = array('success' => true, 'userId' => $_SESSION['userId'],'modules' => $_SESSION["modules"], 'serviceInfo' => $_SESSION["serviceInfo"]);
            if (isset($_SESSION['wxAccountId'])) {
                $result['wxAccountId'] = $_SESSION['wxAccountId'];
            }
        } else if ($clearCache) {
            // only clear cache, not really forward request.
            unset($_SESSION[$key]);
