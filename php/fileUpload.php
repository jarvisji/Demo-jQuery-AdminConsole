...
$bucket = 'freecoder-console-user-files';
...
function upload2Bcs($overwrite = false) {
    global $file, $baiduBcs, $bucket, $result, $host, $path, $targetFileName;
    $filename = $file['tmp_name'];
    // make sure object name follow BCS rules.
    // if $targetFileName is set, upload file using the name.
    if (isset($targetFileName)) {
        $fileNameArr = explode(".", $file['name']);
        $fileNameArr[0] = $targetFileName;
        $object = "/" . $path . implode(".", $fileNameArr);
    } else {
